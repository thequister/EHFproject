...
tbl_sum_genpop_1<- 
  ...
  select(ehf_aware_pretr, age_clean, male, main_job, 
         ...
         ehf_prior_know, emergency_expense, new_job, ehf_offer_percent, attachment_index) |> 
  gtsummary::tbl_summary(missing_text = "(missing/not gathered)",
                         label = list( 
                           ...
                           hourly ~ "hourly", college ~ "college degree", member_union ~ "union member",
                           ehf_prior_know ~ "EHF awareness (post-treatment)", emergency_expense ~ "deal with emergency expense", 
                           new_job ~ "accept new job", ehf_offer_percent ~ "accuracy of ehf ident", attachment_index ~ "attachment index"
                         )
  ) |>  
  gtsummary::modify_caption("**Sample snames(ummary statistics by treatment group**") |> 
