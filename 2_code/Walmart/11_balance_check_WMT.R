...
Bin_t <- round(append(Bin_t, t.test(as.numeric(ACNT_clean$tenure_num[ind]), as.numeric(ACNT_clean$tenure_num[-ind]))$p.value), digits = 4)
...
varnames <- c("Multiple Jobs", "Fulltime", "EHF Pre-treat Awareness", "Voted24", "College", 
              ...
              "Home Owner", "Manager", "Union Member", "Pay per hour", "Ideology (Numeric)", "Tenure")

balance.data <- data.frame(varnames, 
                           tab$Balance[1][c(1:12, 14, 16, 18, 20), ], 
                           round(tab$Balance[2][c(1:12, 14, 16, 18, 20), ], digits = 4), 
                           round(tab$Balance[3][c(1:12, 14, 16, 18, 20), ], digits = 4), 
                           Bin_t)
