...
temp<-here::here('2_code', 'THD', '1_libraries_and_settings.R')
...
Third, in the absence of basic descriptive information about EHFs, we do not yet have a convincing identification strategy for studying the effects of EHF introduction on the affected workers or in an industry.  Instead, I propose a tractable, individual-focused approach using experiments embedded in worker surveys that manipulate the information and salience of EHFs in the minds of respondents.  As this is about information and communication, I am careful to account for respondents who were already aware of the EHF programs (i.e., "pre-exposed") prior to the survey.

Fourth the reasons an employer might build an EHF are distinct from the effects an EHF may have on workers.  Nevertheless, considering plausible reasons for creating an EHF program, as we did above, suggests where to look for EHF effects.  There is no implication that the HF or ACNT were necessarily designed or implemented to achieve the goals of team-building, union avoidance or changing public opinion.  The EHFs could have those effects without such explicit intentions.  Executives could well have had goals for the EHF that the programs fail to achieve.  Even if EHFs were designed to affect workers' attitudes, the program may well go unnoticed or unremembered most of the time.  As such, the research design in this paper speaks to whether a informational campaigns like ones that some employers are actually pursuing _could_ affect worker attitudes, for example by calling attention to an EHF as part of a worker on-boarding process or anti-unionization strategy.
