...
```{r fig-genpop-support, fig.show="hold", out.width="80%", fig.cap="Worker support for creating (L) or maintaining (R) an EHF at current job, by own employer EHF status."}
ehf_support_genpop
```
...
Our indicator of subjective financial security is the standard question from the Survey of Household Economics and Decision-making that asks whether a respondent could cover a $400 emergency expense.  Figure \@ref(fig:fig-finsec) displays the weighted distribution of responses by treatment condition.  The upper panel reports the full distribution of the Likert scale for the response whereas the lower panel dichotomizes the variable based on whether the respondent was able or unable to cover the expense.  Vertical lines represent 95% confidence intervals.  
