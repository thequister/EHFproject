...
- **H5 (Contingency):** The effects of EHF messaging on job attachment and union support will be stronger where the program is more generous and where the program is more deeply emebedded in an organizational culture that emphasizes mutual obligation and teamwork.
...
Before examining the survey experiment, we look compare workers' awareness of and engagement with their employer's EHF at the Home Depot and Walmart.
...
In both studies 2 and 3, I included experiments that randomly exposed respondents to messages about their employer's EHF to test hypothesis 1 and 3-5.
...
Our indicator of subjective financial is the standard question from the Survey of Household Economics and Decision-making that asks whether a respondent could cover a $400 emergency expense.  Figure \@ref(fig:fig-finsec) displays the weighted distribution of responses by treatment condition.  The upper panel reports the full distribution of the Likert scale for the response whereas the lower panel dichotomizes the variable based on whether the respondent was able or unable to cover the expense.  Vertical lines represent 95% confidence intervals.  
...
```{r fig-uv-thd,eval=TRUE, echo=FALSE, fig.cap="Support for unionization by treatment status in the Home Depot sample"}
...
```{r fig-uveff, eval=TRUE, echo=FALSE, fig.height=8, fig.cap="Interpreting heterogeneous treatment effects on union support (Home Depot sample)"}
