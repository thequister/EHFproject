...
mean(dt$emp_reco_num) # 0 is max recommend
...
mean(dt$union_vote_num) # 0 is max for the union
...
summary(as.factor(dt$app_time))
mean(dt$app_time_num, na.rm = T)
median(dt$app_time_num, na.rm = T)

summary(as.factor(dt$delay))
mean(dt$delay_num, na.rm = T)
median(dt$delay_num, na.rm = T)
...
dt %>%
  ...
  colMeans() # with 0 being maximum responsibility
...
mean(dt$practice_religion_num) # numerical, where 1 is never, 0 is once a week
