...
We explore these relationships further using both linear regression and hierarchical linear models.  In the latter, we report three specifications, one allowing for random state-level intercepts and a second that admits cross-level interactions between respondent political conservatism and state-level safety net generosity. Because appreciating real estate assets may depress support for the welfare state, we fit a third model that includes a cross-level interaction between an indicator of homeownership and the 5-year change in the state-level home price index produced by the US Federal Housing Finance Agency. We include a slate of other individual-level covariates connected to charitable giving and the substitution hypothesis: whether the respondent has used welfare in the past, religiosity, and political conservatism [@brooksWhoReallyCares2006; @bekkersWhoGivesLiterature2011; @wiepkingWhoGivesLiterature2012; @andreoniCharitableGiving2013].^[Religiosity is a binary variable measuring if a respondent declared affiliation with any religion. We encode conservatism as binary variable, taking "1" if the respondent placed themselves as "Conservative" or "Leaning Conservative" on a 5-point ideological scale.].  We condition on race, gender, age, education, and household income but do not report parameter estimates for space considerations.
...
```{r, echo=FALSE, out.width="80%", fig.cap="Multimodal similarity: Home Depot treatment vs. Walmart treatment", fig.align="center"}
knitr::include_graphics(
