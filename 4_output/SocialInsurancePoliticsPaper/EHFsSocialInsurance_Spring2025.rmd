...
```{r fig-ui, echo=FALSE, fig.cap = "Government responsibility for supporting the unemployed, by EHF treatment status", fig.subcap = c("Study 1", "Study 2"), out.width="50%", fig.align = "center"}
...
Study 3 included an opportunity to click a link to donate to the ACNT.  As the right panel in Figure \@ref(fig-emerg-donate) shows, there is a low baseline willingness to take this action of `r round(100*donation_summary$prop[1])`% in the Walmart sample.  Nevertheless, those seeing the EHF video were about 4 percentage points (or 35\%, $p < 0.05$) more likely to take an action to donate. 
...
The ACNT treatment did significantly increase willingness to donate, as expected, but there is again no evidence to support the claim that welfare generosity has any effect on the willingness of workers to donate to an EHF program.  
...
Across both the Home Depot and Walmart surveys, we see no evidence that "private welfare" in the form of EHFs has any negative effect on support for government-directed social support, whether for childcare, the unemployed, the elderly, or those experiencing sudden hardship.
